...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public BaseDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		Logger.debug(TAG, "clazz:" + this.clazz + " tableName:" + this.tableName
	...
	public T get(int id) {
		...
		Logger.debug(TAG, "[get]: select * from " + this.tableName + " where "
				+ this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs) {
		Logger.debug(TAG, "[rawQuery]: " + getLogSql(sql, selectionArgs));
		...
		try {
			...
			Logger.error(this.TAG, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		Logger.debug(TAG, "[isExist]: " + getLogSql(sql, selectionArgs));
		...
		try {
			...
			Logger.error(this.TAG, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public List<T> find(String[] columns, String selection,
		...
		try {
			...
			Logger.error(this.TAG, "[find] from DB Exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public void saveOrUpdate(T entity) throws Exception {
		Method idMethod = entity.getClass().getMethod("get" + getMethodName(this.idColumn));
		Object idObject = idMethod.invoke(entity);
		if(idObject != null) {
			T t = null;
	...
	public void saveOrUpdate(T entity, boolean generateId) throws Exception {
		Method idMethod = entity.getClass().getMethod("get" + getMethodName(this.idColumn));
		Object idObject = idMethod.invoke(entity);
		if(idObject != null) {
			T t = null;
	...
	public long insert(T entity, boolean flag) {
		...
		try {
			...
			Logger.debug(TAG, "[insert]: insert into " + this.tableName + " " + sql);
			long row = db.insert(this.tableName, null, cv);
			return row;
		} catch (Exception e) {
			Logger.debug(this.TAG, "[insert] into DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
	...
	public void delete(int id) {
		...
		Logger.debug(TAG, "[delete]: delelte from " + this.tableName + " where "
	...
	public void delete(Integer... ids) {
		if (ids.length > 0) {
			...
			Logger.debug(TAG, "[delete]: " + getLogSql(sql, ids));
	...
	public void update(T entity) {
		...
		try {
			...
			Logger.debug(TAG, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", id));

			String[] whereValue = { id };
			db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			Logger.debug(this.TAG, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null)
				db.close();
...
*/
	...
	public List<Map<String, String>> query2MapList(String sql,
		...
		Logger.debug(TAG, "[query2MapList]: " + getLogSql(sql, selectionArgs));
		SQLiteDatabase db = null;
		Cursor cursor = null;
		...
		try {
			...
			Logger.error(TAG, "[query2MapList] from DB exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		SQLiteDatabase db = null;
		Logger.debug(TAG, "[execSql]: " + getLogSql(sql, selectionArgs));
		try {
			db = this.dbHelper.getWritableDatabase();
			...
			Logger.error(TAG, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
	...
	public void clear() {
		...
		db.delete(tableName, null, null);
			...
			Logger.error(TAG, "[clear] DB exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
	 ...
	 * @return
	 */
	public boolean isExistIncludTransaction(SQLiteDatabase db,String sql, String[] selectionArgs) throws Exception{
	 ...
	 * @return
	 */
	public long insertIncludTransaction(SQLiteDatabase db,T entity) throws Exception {
		return insertIncludTransaction(db,entity, false);
	}
	/**
	 * 事务中--插入（db未关闭）
	 * @param db
	 ...
	 * @throws Exception
	 */
	public long insertIncludTransaction(SQLiteDatabase db,T entity, boolean flag) throws Exception{
		String sql = "";
	 ...
	 * 事务中--删除（db未关闭）
	 */
	public void deleteIncludTransaction(SQLiteDatabase db,int id) {
		if(db == null)return;
		String where = this.idColumn + " = ?";
		String[] whereValue = { Integer.toString(id) };
	 ...
	 * 事务中--删除（db未关闭）
	 */
	public void deleteIncludTransaction(SQLiteDatabase db,String id) {
		if(db == null)return;
		String where = this.idColumn + " = ?";
		String[] whereValue = { id };
	 ...
	 * 事务中--删除（db未关闭）
	 */
	public void deleteIncludTransaction(SQLiteDatabase db,Integer... ids) {
		if(db == null)return;
		if (ids.length > 0) {
			StringBuffer sb = new StringBuffer();
			for (int i = 0; i < ids.length; i++) {
	 ...
	 * @param whereValue
	 */
	public void updateIncludTransaction(SQLiteDatabase db,T entity, String where, String[] whereValue) throws Exception{
		if(db == null)return;
...
//			cv.remove(this.idColumn);
			...
			Log.d(TAG, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(whereValue)));
...
//			String[] whereValue = { Integer.toString(id) };
	 ...
	 * 事务中--更新（db未关闭）
	 */
	public void updateIncludTransaction(SQLiteDatabase db,T entity) throws Exception{
		if(db == null)return;
			ContentValues cv = new ContentValues();
