 ...
 * @company 金鑫智慧
 */
public class CustomerPatternCMDList extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public CustomerPatternCMDList(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<CustomerPatternCMD> customerPatternCMDList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					customerPatternCMDList = new ArrayList<CustomerPatternCMD>();
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						customerPatternCMDList.add(new CustomerPatternCMD(
								getJsonInt(_jo, "id"), getJsonInt(_jo,
										"patternId"), getJsonInt(_jo, "cmdId"),
								getJsonInt(_jo, "exeOrder")));
					}

					// /////////////////////////////////////
					isSuccess = true;
					SharedDB.saveStrDB(_account,ControlDefine.
							KEY_CUSTOMER_PATTERN_CMD_LIST,_processTime);
				}
				...
				if (isSuccess) {
					this.task.callback(customerPatternCMDList);
				} else {
