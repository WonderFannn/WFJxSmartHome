 ...
 * @company 金鑫智慧
 */
public class WakeWordRj extends ResponseJson {

	private Task task = null;
	private byte[] requestBytes;

	public WakeWordRj(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if(in != null){
			...
			List<WakeWord> wakeWordList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					wakeWordList = new ArrayList<WakeWord>();
					
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						wakeWordList.add(new WakeWord(getJsonInt(_jo, "id"),
								getJsonString(_jo, "customerId"),getJsonString(_jo, "wakeUpWord"),
								getJsonString(_jo, "status"), getJsonDouble(_jo, "operateTime")));
					}
				...
				e.printStackTrace();
				 ...
				 if(isSuccess){
					 this.task.callback(wakeWordList);
				 }else{
