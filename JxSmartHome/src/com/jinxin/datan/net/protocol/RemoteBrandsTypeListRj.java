 ...
 * @company 金鑫智慧
 */
public class RemoteBrandsTypeListRj extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public RemoteBrandsTypeListRj(Task task, byte[] requestBytes) {
		this.task = task;
		...
		if (in != null) {
			...
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					isSuccess = true;
					SharedDB.saveStrDB(_account,ControlDefine.KEY_REMOTE_BRANDS,_processTime);
				}
