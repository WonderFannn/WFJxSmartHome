 ...
 * @company 金鑫智慧
 */
public class CustomerTimerList extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public CustomerTimerList(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<CustomerTimer> customerTimerList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					customerTimerList = new ArrayList<CustomerTimer>();
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						customerTimerList.add(new CustomerTimer(getJsonInt(
								_jo, "taskId"), 
								getJsonString(_jo,"customerId"),
								getJsonString(_jo,"taskName"),
								getJsonString(_jo,"period"),
								getJsonString(_jo,"cornExpression"),
								getJsonInt(_jo, "status"),
								getJsonString(_jo, "updateTime")));
					}
					isSuccess = true;
					SharedDB.saveStrDB(_account,
							ControlDefine.KEY_CUSTOMER_TIMER_LIST,
							_processTime);
				}
				...
				if (isSuccess) {
					this.task.callback(customerTimerList);
				} else {
