 ...
 * @company 金鑫智慧
 */
public class ProductPatternOperationList extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public ProductPatternOperationList(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<ProductPatternOperation> productPatternOperationList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					productPatternOperationList = new ArrayList<ProductPatternOperation>();
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						productPatternOperationList
								.add(new ProductPatternOperation(getJsonInt(_jo, "id"),getJsonInt(
										_jo, "funId"), getJsonString(_jo,
										"whId"), getJsonInt(_jo, "patternId"),
					...
					SharedDB.saveStrDB(_account,
							ControlDefine.KEY_PRODUCT_PATTERN_OPERATION_LIST,
							_processTime);
				}
				...
				if (isSuccess) {
					this.task.callback(productPatternOperationList);
				} else {
