...
public class VoiceToTextRj extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;
	
	public VoiceToTextRj(Task task, byte[] requestBytes) {
		this.task = task;
		...
		if (in != null) {
			...
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					// 具体的解析工作在这里实现
					JSONObject _jb = jsonObject.getJSONObject("serviceContent");
					result = _jb.getString("resultCode");
