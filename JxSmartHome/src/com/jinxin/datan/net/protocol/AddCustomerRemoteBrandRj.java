 ...
 * @company 金鑫智慧
 */
public class AddCustomerRemoteBrandRj extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;
	
	public AddCustomerRemoteBrandRj(Task task, byte[] requestBytes) {
		this.task = task;
		...
		if (in != null) {
			...
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					remoteId = jsonObject.getInt("serviceContent");
					isSuccess = true;
				}
