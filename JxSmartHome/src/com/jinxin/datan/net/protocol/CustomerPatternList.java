 ...
 * @company 金鑫智慧
 */
public class CustomerPatternList extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public CustomerPatternList(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<CustomerPattern> customerPatternList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					customerPatternList = new ArrayList<CustomerPattern>();
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						customerPatternList.add(new CustomerPattern(getJsonInt(
								_jo, "patternId"), getJsonString(_jo,
								"paternName"),getJsonString(_jo,
										"paternType"),
								getJsonString(_jo, "customerId"),
								getJsonString(_jo, "createUser"),
								getJsonString(_jo, "createTime"),
								getJsonString(_jo, "status"),
								getJsonString(_jo, "icon"),
								getJsonString(_jo, "ttsStart"),
								getJsonString(_jo, "ttsEnd"),
								getJsonInt(_jo, "patternGroupId"),
								getJsonString(_jo, "memo")));
					}
				...
				if (isSuccess) {
					this.task.callback(customerPatternList);
				} else {
