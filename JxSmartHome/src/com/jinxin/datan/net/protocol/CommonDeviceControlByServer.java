 ...
 * @company 金鑫智慧
 */
public class CommonDeviceControlByServer extends ResponseJson {
	...
	public CommonDeviceControlByServer(Task task, byte[] requestBytes, int requestType) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		/**** test *********/
		if (in != null) {
			Logger.warn(null, "parse response for normal:" + cmdCount);
...
//			String tempStr = "";
			try {
				...
				for (int i = 0; i < this.cmdCount; i++) {
					...
					_cmdr.setLen(len);
					System.out.println("online len:" + len);
					if (len != 0) {
						//长度过大不进行解析
						if (len < 1024*10) {
