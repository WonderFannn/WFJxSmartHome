 ...
 * @company 金鑫智慧
 */
public class CustomerDataSyncParser extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public CustomerDataSyncParser(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<CustomerDataSync> cpAreaList = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {
					...
					cpAreaList = new ArrayList<CustomerDataSync>();
...
//					}
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						CustomerDataSync cds = new CustomerDataSync(
								getJsonInt(_jo, "id"), 
								getJsonString(_jo, "customerId"),
								getJsonString(_jo, "actionId"),
								getJsonString(_jo, "actionFieldName"),
								getJsonString(_jo, "actionFieldType"),
								getJsonInt(_jo, "action"),
								getJsonString(_jo, "model"),
					...
					SharedDB.saveStrDB(_account,
							ControlDefine.KEY_CUSTOMER_DATA_SYNC, _processTime);
				}
