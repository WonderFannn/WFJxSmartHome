 ...
 * @company 金鑫智慧
 */
public class MonitorList extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public MonitorList(Task task, byte[] requestBytes) {
		this.task = task;
		...
		if (in != null) {
			...
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {// 成功获取数据
					...
					SharedDB.emptyStrDB(_account, ControlDefine.KEY_MONITOR_LIST);
					for (int i = 0; i < jsonArray.length(); i++) {
						if (this.task.ismTryCancel())
							return;
						JSONObject _jo = (JSONObject) jsonArray.get(i);
						monitorList.add(getJsonString(_jo, "mac"));
					}
					for (String val : monitorList) {
						saveMonitorID(val);
					}

					// /////////////////////////////////////
					isSuccess = true;
					SharedDB.saveStrDB(_account, ControlDefine.KEY_MONITOR_DEVICE, _processTime);
				}
