 ...
 * @company 金鑫智慧
 */
public class ProductRemoteConfigFunParser extends ResponseJson {
	private Task task = null;
	private byte[] requestBytes;

	public ProductRemoteConfigFunParser(Task task, byte[] requestBytes) {
		this.task = task;
	...
	public void response(InputStream in) throws Exception {
		if (in != null) {
			...
			List<ProductRemoteConfigFun> list = null;
			try {
				...
				if (_resultInfo.validResultCode.equals(ControlDefine.CONNECTION_SUCCESS)) {
					...
					list = new ArrayList<ProductRemoteConfigFun>();
...
//					}
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						ProductRemoteConfigFun prcf = new ProductRemoteConfigFun(
								getJsonInt(_jo, "id"), 
								getJsonString(_jo, "customerId"),
								getJsonString(_jo, "funId"),
								getJsonString(_jo, "type"),
								getJsonString(_jo, "configId"),
								getJsonString(_jo, "areaId"),
					...
					SharedDB.saveStrDB(_account,
							ControlDefine.KEY_PRODUCT_REMOTE_CONFIG_FUN, _processTime);
				}
