...
public class BytesEncodingDetector extends Encoding {
	...
	/**
	 * Function : detectEncoding Aruguments: URL Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5, EUC_TW,
	 * ASCII, or OTHER) Description: This function looks at the URL contents and assigns it a probability score for each encoding type. The
	...
	/**
	 * Function : detectEncoding Aruguments: File Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5, EUC_TW,
	 * ASCII, or OTHER) Description: This function looks at the file and assigns it a probability score for each encoding type. The encoding
	...
	/**
	 * Function : detectEncoding Aruguments: byte array Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5,
	 * EUC_TW, ASCII, or OTHER) Description: This function looks at the byte array and assigns it a probability score for each encoding
	...
	public int detectEncoding(byte[] rawtext) {
		...
		for (index = 0; index < TOTALTYPES; index++) {
			if (debug) 
				Logger.error("detectEncoding", "Encoding " + nicename[index] + " score " + scores[index]);
	...
	/*
	 * Function: gb2312_probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array
	 * uses GB-2312 encoding
	 */
	...
	/*
	 * Function: gbk_probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array uses
	 * GBK encoding
	 */
	...
	/*
	 * Function: gb18030_probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array
	 * uses GBK encoding
	 */
			...
			if (rawtext[i] == '~') {
				if (rawtext[i + 1] == '{') {
					...
					while (i < rawtextlen - 1) {
						...
						} else if (((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7)
								&& ((byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xF7)) {
							hzchars += 2;
							row = rawtext[i] + 256 - 0xA1;
							column = rawtext[i + 1] + 256 - 0xA1;
							totalfreq += 500;
	...
	/**
	 * Function: big5_probability Argument: byte array Returns : number from 0 to 100 representing probability text in array uses Big5
	 * encoding
	 */
	...
	/*
	 * Function: big5plus_probability Argument: pointer to unsigned char array Returns : number from 0 to 100 representing probability text
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= (byte) 128) {
				...
				if ((byte) 0xA1 <= rawtext[i]
					...
					bfchars++;
					totalfreq += 500;
					row = rawtext[i] - (byte) 0xA1;
					if ((byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E) {
						column = rawtext[i + 1] - (byte) 0x40;
					} else {
						column = rawtext[i + 1] - (byte) 0x61;
					}
					// System.out.println("original row " + row + " column " +
					// column);
					if (Big5Freq[row][column] != 0) {
						...
						&& rawtext[i] <= (byte) 0xFE
						&& // Extended Big5 range
						(((byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E) || ((byte) 0x80 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE))) {
					bfchars++;
					totalfreq += 500;
					row = rawtext[i] - (byte) 0x81;
					if ((byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E) {
						column = rawtext[i + 1] - (byte) 0x40;
					} else {
						column = rawtext[i + 1] - (byte) 0x40;
	...
	/*
	 * Function: euc_tw_probability Argument: byte array Returns : number from 0 to 100 representing probability text in array uses EUC-TW
	 * (CNS 11643) encoding
	 */
	...
	/*
	 * Function: iso_2022_cn_probability Argument: byte array Returns : number from 0 to 100 representing probability text in array uses ISO
	 * 2022-CN encoding WORKS FOR BASIC CASES, BUT STILL NEEDS MORE WORK
	 */
	...
	/*
	 * Function: utf8_probability Argument: byte array Returns : number from 0 to 100 representing probability text in array uses UTF-8
	 * encoding of Unicode
	 */
	...
	/*
	 * Function: utf16_probability Argument: byte array Returns : number from 0 to 100 representing probability text in array uses UTF-16
	 * encoding of Unicode, guess based on BOM // NOT VERY GENERAL, NEEDS MUCH MORE WORK
	 */
	int utf16_probability(byte[] rawtext) {
		...
		return 0;
		/*
		 * // Check to see if characters fit into acceptable ranges rawtextlen = rawtext.length; for (i = 0; i < rawtextlen; i++) { if
		 * ((rawtext[i] & (byte)0x7F) == rawtext[i]) { // One byte goodbytes += 1; asciibytes++; } else if ((rawtext[i] & (byte)0xDF) ==
		 * rawtext[i]) { // Two bytes if (i+1 < rawtextlen && (rawtext[i+1] & (byte)0xBF) == rawtext[i+1]) { goodbytes += 2; i++; } } else
		 * if ((rawtext[i] & (byte)0xEF) == rawtext[i]) { // Three bytes if (i+2 < rawtextlen && (rawtext[i+1] & (byte)0xBF) == rawtext[i+1]
		 * && (rawtext[i+2] & (byte)0xBF) == rawtext[i+2]) { goodbytes += 3; i+=2; } } }
		 * 
		 * score = (int)(100 * ((float)goodbytes/(float)rawtext.length)); // An all ASCII file is also a good UTF8 file, but I'd rather it
		 * // get identified as ASCII. Can delete following 3 lines otherwise if (goodbytes == asciibytes) { score = 0; } // If not above
		 * 90, reduce to zero to prevent coincidental matches if (score > 90) { return score; } else { return 0; }
		 */
	}
	...
	/*
	 * Function: euc_kr__probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array
	 * uses EUC-KR encoding
	 */
	...
	/*
	 * Function: cp949__probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array
	 * uses Cp949 encoding
	 */
	...
	/*
	 * Function: euc_jp_probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array
	 * uses EUC-JP encoding
	 */
	...
	/*
	 * Function: sjis_probability Argument: pointer to byte array Returns : number from 0 to 100 representing probability text in array uses
	 * Shift-JIS encoding
	 */
