...
/**
 * 手机网络检测的常用方法
 * 		来源：马天宇的android-lite-http项目，地址：https://github.com/chourentang/android-lite-http
 * @author  TangLong
 * @company 金鑫智慧
 */
public class NetworkUtil {
	...
	public static boolean printNetworkInfo(Context context) {
		...
		if (connectivity != null) {
			...
			Logger.info(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
			Logger.info(TAG, "getActiveNetworkInfo: " + in);
			NetworkInfo[] info = connectivity.getAllNetworkInfo();
			if (info != null) {
				for (int i = 0; i < info.length; i++) {
					// if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
					Logger.info(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
					Logger.info(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
					Logger.info(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
					Logger.info(TAG, "NetworkInfo[" + i + "]: " + info[i]);
					// }
				}
				Logger.info(TAG, "\n");
			} else {
				Logger.info(TAG, "getAllNetworkInfo is null");
			}
		}
		return false;
	}
