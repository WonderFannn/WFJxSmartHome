...
public abstract class BaseFragmentActivity extends FragmentActivity {
	...
	protected Button button_save = null;
...
//				//打开登录界面
				...
				if (_mBox == null) {
					_mBox = new MessageBox(BaseFragmentActivity.this, _titleOn,"网络连接变化，是否切换在线模式？",
				...
				_mBox.setOnDismissListener(new OnDismissListener() {
					...
					public void onDismiss(DialogInterface dialog) {
						...
						case MessageBox.MB_OK://点击消失
							...
							OfflineToOnline.changeToOnline(_account, secretkey, BaseFragmentActivity.this);
							break;
						}
					}
				});
				
				if(_mBox != null && !_mBox.isShowing()){
					_mBox.show();
				}
				...
				if (!NetworkModeSwitcher.useOfflineMode(ctx)) {
					...
					if (mb == null) {
						mb = new MessageBox(BaseFragmentActivity.this, _titleOff,"与服务器通信异常，是否启用离线模式",
								MessageBox.MB_OK | MessageBox.MB_CANCEL);
...
//							BroadcastManager.sendBroadcast(BroadcastManager.ACTION_CHANGE_ON_LINE_MODE, null);
								...
								View dialogView = inflater.inflate(R.layout.custom_dialog_pwd_confirm, null);
								...
								final Dialog changDialog = new Dialog(BaseFragmentActivity.this);
								changDialog.setContentView(dialogView);
								changDialog.setTitle(R.string.confirm_secretkey);
									...
									public void onClick(View v) {
										...
										if(!TextUtils.isEmpty(pwdText.getText().toString())){
											...
											if (secretkey.equals(pwd)) {
												NetTool netTool = new NetTool(BaseFragmentActivity.this);
												netTool.scan1();
												changDialog.dismiss();
											}else{
												JxshApp.showToast(ctx,CommDefines.getSkinManager().string(R.string.li_xian_mo_shi_mi_ma_error));
											}
										}
									}
								});
