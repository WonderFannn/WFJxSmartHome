...
public class ModeLightBeltFragment extends DialogFragment  implements OnClickListener, OnSeekBarChangeListener {
	FrameLayout panel;
	...
	private ProductFunVO _productFunVO = null;
...
//		mFadeStop.setOnClickListener(this);
		...
		coolLight.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					_productFunVO.getProductPatternOperation().setOperation(OPERATING_AUTOMODE);
					_productFunVO.getProductPatternOperation().setParaDesc(StringUtils.rgbToHsvJsonForAutoMode());//炫彩灯光参数设置
				}else{//重置彩灯颜色
					_productFunVO.getProductPatternOperation().setOperation(OPERATION_HUEANDSAT);
					_productFunVO.getProductPatternOperation().setParaDesc(StringUtils.rgbToHsvJson(intR, intG, intB));
				}
			}
...
//					showPop();
				...
				case MotionEvent.ACTION_UP:
					...
					_productFunVO.getProductPatternOperation().setParaDesc(_color);
					break;
				}
	...
	private void initData() {
		...
		_productFunVO = (ProductFunVO) getArguments().getSerializable(
				"productFunVO");
		funDetail = (FunDetail) getArguments().getSerializable("funDetail");
		if (_productFunVO != null) {
			productFun = _productFunVO.getProductFun();
			coolLight.setChecked(_productFunVO.
					getProductPatternOperation().getOperation().equals(OPERATING_AUTOMODE));
		}
		if (productFun != null) {
		...
		if (isChanged) {
			...
			if (_productFunVO != null
					&& _productFunVO.getProductPatternOperation() != null) {
				strColor = _productFunVO.getProductPatternOperation()
						.getParaDesc();
