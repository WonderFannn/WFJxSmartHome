 ...
 * @company 金鑫智慧
 */
public class AddTimerTaskListAdapter extends BaseAdapter {
	 ...
	 */
	public AddTimerTaskListAdapter(Context con,List<ProductFunVO> productFunVOList,FunDetail funDetail,int whichMode){
		this.con=con;
...
//							}
			...
			holder.imageButton_open.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					if(_productFunVO.isSelected()){
						if (funDetail.getFunType().equals(ProductConstants.FUN_TYPE_MASTER_CONTROLLER_LIGHT)||
							...
							if (!_productFunVO.isOpen()) {
								...
								if (whichMode == 0) {
									fragmentTransaction = ((AddTimerTaskActivity)con).getSupportFragmentManager().beginTransaction();
								}else{
									fragmentTransaction = ((AmendTimerTaskActivity)con).getSupportFragmentManager().beginTransaction();
								}
					...
					}else if(ProductConstants.FUN_TYPE_THREE_SWITCH_THREE.equals(funDetail.getFunType())||
