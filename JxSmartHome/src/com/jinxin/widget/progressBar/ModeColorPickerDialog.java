 ...
 * 模式添加或修改灯光颜色调节对话框
 */
public class ModeColorPickerDialog extends Dialog {
	 ...
	 *            颜色选择后的回调接口。
	 */
	public ModeColorPickerDialog(Context context, String title,ArrayList<ProductFun> productFunList,FunDetail funDetail,
	 ...
	 *            颜色选择后的回调接口。
	 */
	public ModeColorPickerDialog(Context context, String title,
			int initalColor,ArrayList<ProductFun> productFunList,FunDetail funDetail,
			OnColorChangedListener l) {
		super(context);
		mContext = context;
		mTitle = title;
	...
	protected abstract class ColorPickerView extends View {
		...
		public ColorPickerView(Context context, OnColorChangedListener l) {
			...
			setRGB(Color.alpha(mInitialColor), Color.red(mInitialColor), Color.green(mInitialColor), Color.blue(mInitialColor));
		...
		protected void onActionUp(boolean inCenter) {
			if (mHighlightCenter && inCenter) {// 点击在中心圆, 且当前启动在中心圆
				if (this.mListener != null) {
					...
					ModeColorPickerDialog.this.dismiss();
				}
			}
			if (mDownInCircle) {
				mDownInCircle = false;
...
//		}
	...
	private class PortraitColorPickerView extends ColorPickerView {
		...
		public PortraitColorPickerView(Context context, OnColorChangedListener l) {
			super(context, l);
			Display display = ModeColorPickerDialog.this.getWindow()
