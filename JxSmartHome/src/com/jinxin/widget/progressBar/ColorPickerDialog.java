...
/**
 * @author YangJiJun
 * @company 金鑫智慧 灯光颜色调节对话框
 */
public class ColorPickerDialog extends Dialog {
	private Context mContext;
	private String m_color;
	private ArrayList<ProductFun> mProductFun = null;
	private FunDetail mFunDetail = null;
	...
	private String _g = "";
	private String _b = "";
	private int intA, intR, intG, intB;
	...
	public ColorPickerDialog(Context context, String title,
			ArrayList<ProductFun> productFunList, FunDetail funDetail,
			OnColorChangedListener l) {
		this(context, title, Color.WHITE, productFunList, funDetail, l);
	...
	public ColorPickerDialog(Context context, String title, int initalColor,
		...
		mProductFun = productFunList;
		mFunDetail = funDetail;

	}
	
	public View drawColor(Context context, String title, int initalColor,
			ArrayList<ProductFun> productFunList, FunDetail funDetail){
		View view = null;
		mContext = context;
		mTitle = title;
		...
		if (isPortrait) {
			view = new PortraitColorPickerView(mContext, mListener);
		} else {
			view = new LandscapeColorPickerView(mContext, mListener);
		}
		return view;
	}
	...
	public interface OnColorChangedListener {
		void colorChanged(int color, int a, int r, int g, int b);
	}
	...
	protected abstract class ColorPickerView extends View {
		...
		public ColorPickerView(Context context, OnColorChangedListener l) {
			...
			setRGB(Color.alpha(mInitialColor), Color.red(mInitialColor),
					Color.green(mInitialColor), Color.blue(mInitialColor));
			...
			if (mHighlightCenter && inCenter) {// 点击在中心圆, 且当前启动在中心圆
				if (this.mListener != null) {
					this.mListener.colorChanged(mCenterPaint.getColor(),
							getA(), getR(), getG(), getB());
					m_color = getLightColor();
					intColor = mCenterPaint.getColor();
					if (mProductFun != null) {
						Stack<ProductFun> _ppoStack = new Stack<ProductFun>();
						for (int i = mProductFun.size() - 1; i >= 0; i--) {
							_ppoStack.push(mProductFun.get(i));
						}
						JxshApp.showLoading(
								mContext,
								CommDefines.getSkinManager().string(
										R.string.qing_qiu_chu_li_zhong));
						patternOperationSendToChangColor(_ppoStack, m_color);
	 ...
	 * @param b
	 */
	public synchronized void setRGB(int a, int r, int g, int b) {
		// if (a<10) {
		// _a = "00"+a;
		// }else if (a>=10 && a < 100) {
		// _a = "0"+a;
	...
	private class PortraitColorPickerView extends ColorPickerView {
		...
		public PortraitColorPickerView(Context context, OnColorChangedListener l) {
			...
			mCenterRadius = (mCircleRadius - mCirclePaint.getStrokeWidth() / 2) * 0.8f;
	...
	private class LandscapeColorPickerView extends ColorPickerView {
		...
		public LandscapeColorPickerView(Context context,
			...
			Display display = ColorPickerDialog.this.getWindow()
					.getWindowManager().getDefaultDisplay();
			int height = (int) (display.getHeight() * 0.8f);
	 ...
	 * @param ppos
	 */
	private void patternOperationSendToChangColor(final Stack<ProductFun> pfs,
			final String lColor) {
		if (pfs == null || pfs.empty()) {
			JxshApp.closeLoading();
			return;
		}
		final ProductFun _proFun = pfs.pop();
		if (_proFun != null) {
			List<byte[]> cmdList = CommonMethod.lightFunToCMD(mContext,
					_proFun, mFunDetail, lColor);
			...
			TaskListener<ITask> listener = new TaskListener<ITask>() {
				...
				public void onSuccess(ITask task, Object[] arg) {
					patternOperationSendToChangColor(pfs, lColor);
					_proFun.setLightColor(lColor);// 设置化彩灯颜色
					_proFun.setBrightness(0);// 彩灯默认亮度为0
					_proFun.setIntColor(intColor);// 彩灯默认整形颜色值
					ProductFunDaoImpl _pfDaoImpl = new ProductFunDaoImpl(
							getContext());
					_pfDaoImpl.update(_proFun);
				}
			...
			if(NetworkModeSwitcher.useOfflineMode(mContext)) {
				OfflineCmdSenderLong offlineSender = new OfflineCmdSenderLong(mContext, cmdList);
			...
			if (pfs == null || pfs.empty()) {
				...
				if (cdcbsTask != null) {
					...
					JxshApp.showToast(mContext, CommDefines.getSkinManager()
